...
class HandTracker:
    ...
    def __init__(
        ...
        self.new_track_conf = max(0.0, float(new_track_conf))
    ...
    def update_detections(self, boxes: List[List[float]],
        ...
        # 重要：匹配列表和轨迹对象必须保持同一索引空间。旧实现只取
        # active 的框，却用 active 下标访问 self.tracks，丢帧轨迹夹在
        # 中间时会把新框写入另一只手。
        ...
        # 持旧框）：左 track 在右手碎片框上续命会骨架跳手。默认 None
        # 关；手套链传 0.7，与新建 track 的碎片拒绝同口径。
        ...
        for curr_idx in unmatched_curr:
            # 低置信闪框只允许维持已有轨迹，不能凭空创建新 ID。
            if (self.new_track_conf > 0.0 and confs is not None
                    and curr_idx < len(confs)
                    and float(confs[curr_idx]) < self.new_track_conf):
                continue
            # YOLO-World 可能对同一只手产生碎片/重复框，拒绝完全落在
            # 已匹配手框内的候选，避免挤掉健康轨迹。
...
def _contained(cand: List[float], refs: List[List[float]],
    ...
    """判断候选框是否 ≥frac 面积落在某个已有框内。"""
    cx1, cy1, cx2, cy2 = cand
    area = max((cx2 - cx1) * (cy2 - cy1), 1e-6)
    for rx1, ry1, rx2, ry2 in refs:
